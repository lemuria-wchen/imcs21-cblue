...
class MyModel(object):
    ...
    def __init__(self, 
        ...
        with tf.variable_scope('encoder'):
            ...
            rnn_outputs = tf.add(rnn_fw_outputs, rnn_bw_outputs) # B * S * D(2*hidden_dim)

        # bio预测层
        with tf.variable_scope('bio_projection'):
            logits_bio = tf.layers.dense(rnn_outputs, vocab_size_bio) # B * S * V(BIO类别)
        ...
        with tf.variable_scope('attr_projection'):
            logits_attr = tf.layers.dense(rnn_outputs, vocab_size_attr) # B * S * V(attr类别)
        ...
        with tf.variable_scope('type_projection'):
            logits_type = tf.layers.dense(rnn_outputs, vocab_size_type) # B * S * V(attr类别)
